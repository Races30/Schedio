...
export default function EmployeePage() {
  ...
  // Parse token from URL: /:slug/:name--token
  ...
  if (!employee || !activity) {
    return <div className="min-h-screen flex items-center justify-center p-4"><div className="text-center"><h1 className="text-2xl font-bold mb-2">Pagina non trovata</h1><p className="text-muted-foreground">Il link non è valido.</p></div></div>;
  }
            ...
            {hours.map(time => (
              <div key={time} className="grid grid-cols-[80px_repeat(7,1fr)] border-b border-border/50">
                ...
                {weekDays.map((day, di) => {
                  ...
                  return (
                    <div key={di} className={`border-l border-border/50 min-h-[48px] p-0.5 ${isNow ? 'bg-primary/5' : ''}`}>
                      {slotAppts.filter(a => a.start_time === time).map(appt => {
                        ...
                        return (
                          <div key={appt.id}
                            ...
                            style={{ backgroundColor: (appt.color || employee.color) + '20', height: `${durationSlots * 48 - 4}px`, position: 'relative', zIndex: 10 }}>
                            <div className="font-medium truncate" style={{ color: appt.color || employee.color }}>
